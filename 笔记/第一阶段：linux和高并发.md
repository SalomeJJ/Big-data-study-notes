...
* `grep "ooxx" grep.txt`  显示grep.txt里包含ooxx的行
  ...
  ![1549006112593](assets/1549006112593.png)

* `grep "[0-9]" grep.txt` 显示包含数字的行

  ![1549006272860](assets\1549006272860.png)

  

* `grep "[34]" grep.txt` 显示包含数字3 **或** 4的行   （单字符占位）

  ![1549006460537](assets\1549006460537.png)

* `grep "[0-9]\{4\}" grep.txt` 显示包含四位整数的行 注意 {}是拓展正则表达式，要加 \

  ![1549011488982](assets\1549011488982.png)

* `grep -E "[0-9]{4}" grep.txt` 和上面一样的效果，不需要加 \

* `grep "\<ooxx\>" grep.txt` 包含单词ooxx的行

  ![1549011984713](assets\1549011984713.png)

* `grep "[^0-9][0-9]\{4\}[^0-9]" grep.txt` 只包含四位数字的行（不能多）

  ​	![1549012404502](assets\1549012404502.png)

* `grep -E "(^[0-9]|[^0-9][0-9])[0-9]{2}([0-9][^0-9]|[0-9]$)"  grep.txt`

  与上面比较

  ![1549073410683](assets\1549073410683.png)

* `grep -E "(^[0-9]{4}[^0-9]|[^0-9][0-9]{4}[^0-9]|[^0-9][0-9]{4}$|^[0-9]{4}$" grep.txt`

  与上一种进行比较

  ![1549073180769](assets\1549073180769.png)

* `grep "\<aaa" test`

  `grep "\<aaa\>" test`

  ![1549075781023](assets\1549075781023.png)
...
* `grep -E ".*(god).*(good).*\1.*\2" test`
  ...
  ![1549076372876](assets\1549076372876.png)
...
​	![1549077588898](assets\1549077588898.png)

​	

​	`cut -d" " -f1,2 grep.txt`

​	![1549077765907](assets\1549077765907.png)

​	

​	`cut -d" " -s -f1-3 grep.txt`  -s不显示没有分隔符的行

​	![1549077929949](assets\1549077929949.png)

​	

​	`cut -d" " -s -f3 grep.txt`

​	![1549078087786](assets\1549078087786.png)

​	

​	`cut -d':' -f1 passwd` 取出第一列

​	![1549082891018](assets\1549082891018.png)
...
​	![1549083556335](assets\1549083556335.png)

​	

​	`sort -t" " -k2 -n sort.txt` 按后面的数值大小进行排序

​	![1549083773961](assets\1549083773961.png)



​	`sort -t" " -k2 -n -r sort.txt` 按数值逆序

​	![1549111183059](assets\1549111183059.png)

​	

​	`sort -t" " -n -k1 sort.txt` 如果把第一列非数字按数值排序.......

​	![1549111655959](assets\1549111655959.png)
...
​	![1549111815354](assets\1549111815354.png)

​	![1549112028416](assets\1549112028416.png)
...
![1549173033929](assets\1549173033929.png)

`sed '3d' sort.txt` 删除第三行（源文件不变）

`sed -i '3d' sort.txt` 删除第三行（改变源文件）

![1549173700745](assets\1549173700745.png)
...
![1549174068338](assets\1549174068338.png)

![1549174192930](assets\1549174192930.png)

`sed "s/jiyuan/salome/" sort.txt ` 查找替换

![1549174541354](assets\1549174541354.png)

`sed '/jiyuan/d' sort.txt `  删除符合条件的指定行 （包含"jiyuan"的行）

![1549174750527](assets\1549174750527.png)



`sed 's/\(id:\)3\(:initdefault:\)/\14\2/' inittab`   把3改成4（用到了正则里的反向引用）

![1549176752788](assets\1549176752788.png) 

![1549176771395](assets\1549176771395.png)
...
![1549179803344](assets\1549179803344.png)
...
* `cat passwd` 打开passwd文件
  ...
  ![1549682997093](assets\1549682997093.png)
...
* 需求1：只是显示/etc/passwd的账户:CUT     （第一列）
  ...
  ![1549702579593](assets\1549702579593.png)
...
* 只是显示/etc/passwd的账户和账户对应的shell,而账户与shell之间以制表符分割,而且在所有行开始前添加列名name,shell,在最后一行添加"blue,/bin/nosh"（cut，sed）
  ...
  ![1549703152468](assets\1549703152468.png)

* 搜索/etc/passwd有root关键字的所有行

  `awk '/root/ { print $0}' passwd` $0表示这一行的所有内容

  ![1549703407961](assets\1549703407961.png)

* 统计/etc/passwd文件中，每行的行号，每行的列数，对应的完整行内容

  `awk -F ':' '{print NR "\t" NF "\t" $0}' passwd` 

  ![1549704291419](assets\1549704291419.png)
...
![1549957716801](assets\1549957716801.png)
...
![1549958197824](assets\1549958197824.png)
...
![1549958513446](assets\1549958513446.png)
